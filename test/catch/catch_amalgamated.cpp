...
TestCaseInfo::TestCaseInfo(std::string const& _className, NameAndTags const& _nameAndTags,
                           SourceLineInfo const& _lineInfo)
...
void IStreamingReporter::listTests(std::vector<TestCaseHandle> const& tests,
    ...
    for (auto const& test : tests) {
        ...
        if (config.verbosity() >= Verbosity::High) {
            Catch::cout()
                << TextFlow::Column(Catch::Detail::stringify(testCaseInfo.lineInfo)).indent(4)
                << std::endl;
        }
        ...
        if (!m_isOpen) {
            ...
            *this << RowBreak();
            ...
            for (auto const& info : m_columnInfos) {
                headerCols +=
                    TextFlow::Column(info.name).width(static_cast<std::size_t>(info.width - 2));
                headerCols += spacer;
            }
            m_os << headerCols << '\n';
